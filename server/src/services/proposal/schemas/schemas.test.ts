...
test("draftDataSchema: acepta un draft válido", () => {
  const r = draftDataSchema.safeParse(validDraft);
  assert.equal(r.success, true);
});

test("draftDataSchema: rechaza si falta el nombre del cliente", () => {
  const bad = { ...validDraft, cliente: { dirigidoA: "x", ciudad: "y" } };
  assert.equal(draftDataSchema.safeParse(bad).success, false);
});

test("draftDataSchema: rechaza campos desconocidos (.strict)", () => {
  const bad = { ...validDraft, campoExtra: 1 };
  assert.equal(draftDataSchema.safeParse(bad).success, false);
});
