...
/**
 * Avisa a usuarios OPERACIONES + ADMIN que un proyecto está listo para
 * planificación de instalación. Idempotente: el `uniqueKey` por usuario
 * garantiza que aunque se complete la etapa de Ingeniería Y se apruebe el EFP,
 * cada destinatario recibe la notificación una sola vez de por vida.
 */
export async function notifyEngineeringCompleted(params: {
  ...
  try {
    ...
    for (const user of recipients) {
      ...
      if (!created) continue; // ya se había avisado a este usuario
