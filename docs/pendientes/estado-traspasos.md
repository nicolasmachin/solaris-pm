...
| **A-uteSync (C10)** | **Reconciliar ute-sync con el pipeline nuevo** | `ute-sync.service.ts` busca `HABILITACION_UTE`/`POSTVENTA` para el auto-avance; los proyectos nuevos usan `TRAMITACION_UTE`/`POST_HABILITACION`. Actualizar + evitar doble avance con el efecto de T8. |
