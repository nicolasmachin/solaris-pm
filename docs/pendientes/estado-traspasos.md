...
| ~~B2~~ ✅ | **Motor de completitud de sub-tareas (C4) — HECHO (2026-07-11)** | El traspaso de cierre se dispara desde `syncStageProgress` (`project.service.ts`) cuando la etapa transiciona a COMPLETED por sus sub-tareas resueltas (`COMPLETED` **o** `NO_APLICA`); recibe `actorUserId` (quien resolvió la última) vía `refreshStageProgressAndProject` y los endpoints de subetapa/checklist/complete/complete-all. `NO_APLICA` ahora cuenta como resuelta en `calculateStageProgress`/`syncStageProgress`/`calculateSubstageProgress` (antes trababa la etapa). Dispara principal + extra (`STAGE_TO_TRASPASO` + `STAGE_TO_TRASPASO_EXTRA`, o sea VALIDACION_OPERACIONES → T3+T4). **No se modela "obligatoria" por subetapa**: toda subetapa activa es obligatoria, NO_APLICA es el escape. El bloque manual (`PATCH stage {status:COMPLETED}`) se mantiene como force-complete (idempotente, también dispara extra). E2E verificado (NO_APLICA cierra Onboarding + genera T1 PENDIENTE). Tests: `stage-progress.test.ts` (6). **Gap T8 CERRADO (2026-07-12):** `regenerateUteSubstages(tx, ute, actorUserId?)` ahora propaga el actor a `syncStageProgress`, así al finalizar el trámite UTE (botón `/ute/finalizar` **o** PATCH `/ute-processes/:id` que carga `finalizedAt`) la etapa TRAMITACION_UTE cierra y dispara **T8** (→Experiencia Solar, `PENDIENTE_CONFIRMACION`). Idempotente (verificado: 2 finalizar → 1 solo T8). Caveat: proyectos legacy `HABILITACION_UTE` no disparan T8 (el mapa solo tiene `TRAMITACION_UTE`; son 3, ya finalizados). Rutas de creación de UTE (no finalizan) no pasan actor. |
