...
**El capítulo 2 lo lee todo el mundo.** Son cinco reglas que valen para todos y
son las que hacen que el resto funcione.

---

## 1 · El recorrido completo

Un cliente pasa por ocho etapas, desde que firma hasta que su instalación queda
habilitada:
...
En paralelo, y desde el primer día, **Experiencia Solar acompaña al cliente**. Su
recorrido se divide en tres tramos:
...
> **El capataz asignado a la obra y Experiencia Solar. Nadie más.**
...
Decirle *«eso lo tenés que hablar con el capataz»* o *«ese tema es de
...
para dejar materiales y para la obra.
...
Al **cerrar cada día de obra**, el sistema hace una sola pregunta:

> ### «¿Qué le decimos al cliente si pregunta hoy?»

Y se contesta **de un toque**:
...
**No es un reporte de avance técnico.** No hace falta decir cuántos paneles se
subieron ni si se cambió un cable. Es lo que hay que poder contestarle al cliente
si llama.

**La última opción es la que más importa:** es la que tiene que llegarle a
...
| Algo del cliente que no es de obra | Experiencia Solar |
...
1. **La conversación de expectativa inicial** — el recorrido completo con plazos
   ...
   se busca es que el cliente pueda repetir con sus palabras cuánto va a demorar
   y por qué. **Sin esto, todo el silencio posterior se lee como abandono.**
...
| Para… | Escribile a… |
|---|---|
| Horarios, accesos y materiales del día de obra | **{capataz}** — {teléfono} |
| Todo lo demás | **Alejandra** — {teléfono} |
...
1. Se presenta como **«este es tu equipo»**, nunca como «estas son nuestras
   áreas».
2. **El escalamiento va como excepción**, no como una columna más de la tabla. Si
...
> «Te escribo para contarte que tu trámite sigue en curso, sin novedades todavía;
> apenas haya algo te aviso.»
...
Si un reclamo pasa a un área técnica, **el cliente igual tiene que escuchar algo
ese día**: «ya lo estamos viendo con el equipo técnico».

### Por qué conviene que entren por el portal

Un reclamo por WhatsApp le llega a **una persona**. Si esa persona está de
licencia, el reclamo no existe. Por el portal queda registrado, se mide, y nadie
tiene que acordarse.
...
**Las genera el sistema solo** y aparecen en el portal del cliente:

| Encuesta | Cuándo |
|---|---|
| **Instalación** | Al cerrarse la obra |
| **Habilitación** | Al finalizar el trámite |
| **Aniversario** | Cada año de la puesta en marcha |

Tres preguntas cada una y **solo la primera es obligatoria**. El puntaje es el
promedio.

**Si alguna respuesta es baja**, Experiencia Solar recibe un aviso para hacer
seguimiento. Cada nota baja genera el suyo: un cliente reiteradamente
disconforme no se silencia después del primero.

> **Lo que el sistema no hace es avisarle al cliente que tiene una encuesta.** Por
> eso hay un paso para eso. Sin ese aviso, la encuesta se queda en el portal sin
> que nadie la vea.
...
- **A la izquierda**: el recorrido en tres etapas. Se hace clic en una y aparecen
  sus pasos, con el botón de **Plantillas** de esa etapa y el de **Completar los
  N** si quedan pendientes.
- **A la derecha**: **todo el historial**, lo más nuevo arriba, con el nombre del
  área de donde salió cada cosa. Arriba de todo, el cuadro para registrar un
  contacto nuevo.
...
columna cada una, ordenada por días sin contacto.

**Dos señales, y significan cosas distintas:**
...
**Se le crea el acceso desde el paso «Envío del acceso al portal»**, con el ícono
que está en el propio paso. **No hace falta que tenga mail**: si no lo hay, el
sistema usa su cédula o le arma un usuario con su nombre. Al terminar, la
plantilla del mensaje sale con el usuario y la contraseña adentro.

> Si el cliente **ya tenía acceso**, la contraseña no se puede recuperar. Para
> mandársela hay que resetearla con el botón de reenviar.
...
**Vencer no bloquea nada.** No frena la obra ni el trámite: se ve, se hace, y
...
**No es un impedimento** para darle acceso al portal. Se le crea igual con la
cédula o con un usuario armado con su nombre.
