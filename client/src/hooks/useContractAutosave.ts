// Autosave del borrador del generador de contrato. Mismo comportamiento que el
// de propuestas: debounce 1.5s, sin solapamiento, retry exponencial, y `savedTick`
// para sincronizar el preview.
...
export function useContractAutosave(params: {
  projectId: string;
  ...
  const latest = useRef<ContractData | null>(data);
  latest.current = data;
  ...
  const doSave = useCallback(async () => {
    ...
    setStatus("saving");
    try {
      await contractApi.putDraft(projectId, latest.current);
