// Preview del PDF de la proforma con debounce (se refresca cuando el autosave
// confirma, escuchando `savedTick`). Trae el PDF como blob (auth) y arma un object
// URL para el iframe.
...
export function useProformaPreview(params: { projectId: string; savedTick: number; enabled: boolean }) {
  ...
  const fetchPreview = useCallback(async () => {
    setStatus("loading");
    try {
      const blob = await proformaApi.getDraftPreviewBlob(projectId);
