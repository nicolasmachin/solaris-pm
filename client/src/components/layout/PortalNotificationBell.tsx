...
export function PortalNotificationBell() {
  ...
  function handleItemClick(id: string, projectId: string | null, read: boolean, type: NotificationType) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="relative p-2 rounded-md hover:bg-[var(--color-bg-card-hover)] transition-colors text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]"
      ...
      {open && (
        <div
          className="absolute right-0 top-full mt-2 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-lg shadow-2xl z-50"
        ...
        >
          ...
          <div className="max-h-80 overflow-y-auto">
            {isLoading ? (
              ...
              <p className="text-xs text-[var(--color-text-muted)] text-center py-8">Sin notificaciones</p>
            ) : (
              recent.map((n) => (
                <div
                  key={n.id}
                ...
                >
                  <div className="flex items-start gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-medium text-[var(--color-text-primary)] leading-snug">{n.title}</p>
                      {n.message && (
                        <p className="text-[11px] text-[var(--color-text-secondary)] mt-0.5 leading-snug">{n.message}</p>
                      )}
                      <p className="text-[10px] text-[var(--color-text-muted)] mt-0.5">{relativeTime(n.createdAt)}</p>
                    </div>
                    {!n.read && <span className="w-2 h-2 rounded-full bg-[var(--color-accent)] flex-shrink-0 mt-1.5" />}
                  </div>
                </div>
              ))
            )}
