...
// Columna de preview: iframe con el PDF de la proforma + overlays de estado.
export function ProformaPreview({
  ...
  return (
    <div className="relative h-full overflow-hidden rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)]">
      {blobUrl ? (
        <iframe title="Preview de la proforma" src={blobUrl} className="h-full w-full bg-white" />
      ) : null}
